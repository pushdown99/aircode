/*
 *	Porting By Hell Maker - Duke Kim
 *
 */
#include <unistd.h>
#include <dlog.h>
...
LiquidCrystal_I2C::LiquidCrystal_I2C(unsigned char lcd_addr, unsigned char lcd_cols, unsigned char lcd_rows, unsigned char charsize)
{
    ...
    //	Open I2C Address
    //
    int	ret = peripheral_i2c_open(1, _addr, &m_i2c_h);
    if (ret != PERIPHERAL_ERROR_NONE) {
    	m_i2c_h = NULL;
    	dlog_print(DLOG_ERROR, LOG_TAG, "PERIPHERAL_ERROR");
    }
}
...
void LiquidCrystal_I2C::begin()
{
    ...
    usleep(50);	//	wait_ms(50);

    // Now we pull both RS and R/W low to begin commands
    expanderWrite(_backlightval);   // reset expanderand turn backlight off (Bit 8 =1)
    usleep(1000);	//	wait_ms(1000);
    ...
    write4bits(0x03 << 4);
    usleep(4500);	//	wait_us(4500); // wait min 4.1ms
...
void LiquidCrystal_I2C::clear()
{
    ...
    usleep(2000);	//	wait_us(2000);  // this command takes a long time!
}
...
void LiquidCrystal_I2C::expanderWrite(unsigned char _data)
{
    ...
    //	Write I2C
    //
    int ret = peripheral_i2c_write(m_i2c_h,  data_write, 1);
    if (ret < 0) {
           return;
    }
}
...
{
  dlog_print(DLOG_ERROR, LOG_TAG, "LiquidCrystal_I2C::print %s", text);
  while (*text !=0) {
    //_putc(*text);
    send(*text, Rs);
    text++;
